...
class TestStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                pass#self.log('BUY EXECUTED, %.2f' % order.executed.price)
            elif order.issell():
                self.buyorders.append(self.buy(price=order.executed.price-50, exectype=bt.Order.Limit))
    ...
    def next(self):
        ...
        orderlen = len(self.sellorders)
        if orderlen != 0:
            #上次订单未成交则取消
            sellorder = self.sellorders[orderlen - 1]
            if sellorder.status in [sellorder.Submitted, sellorder.Accepted]:
                self.cancel(sellorder)

        sellprice = self.dataclose[0] + (self.datahigh[0] - self.datalow[0]) * self.params.k / 100.0
        #self.order.append(self.buy())
        self.sellorders.append(self.sell(price=sellprice, exectype=bt.Order.Limit))
