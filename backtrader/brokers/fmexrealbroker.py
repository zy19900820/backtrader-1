...
from backtrader.fmex import Fmex
...
class FmexRealBroker(bt.BrokerBase):
    '''Broker Simulator
    ...
    fmexinterface = bt.fmex.Fmex()
    fmexinterface.auth(conf.key, conf.secret)

    def __init__(self):
        super(FmexRealBroker, self).__init__()
        self._userhist = []
        self._fundhist = []
    ...
    def init(self):
        super(FmexRealBroker, self).init()
    ...
    def cancel(self, order, bracket=False):
        ...
        self.fmexinterface.cancel_order(order.ref)
        order.cancel()
        self.notify(order)
